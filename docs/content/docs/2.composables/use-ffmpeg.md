...
<script setup lang="ts">
  ...
  async function convertVideo() {
    ...
    try {
      ...
      // Convert to MP4
      const result = await ffmpeg.convert(["-preset", "ultrafast", "-crf", "28"])
...
<template>
  <div>
    ...
    <div v-if="ffmpeg.status === 'converting'">Converting... {{ ffmpeg.progress }}s processed</div>

    <div v-if="ffmpeg.status === 'error'" class="text-red-500">Conversion failed</div>

    <button @click="convertVideo" :disabled="isLoading || !videoUrl">Convert to MP4</button>
  </div>
</template>
```
...
<template>
  <div>
    <div class="w-full bg-gray-200 rounded-full h-2.5">
      <div class="bg-primary h-2.5 rounded-full transition-all" :style="{ width: `${progressPercent}%` }" />
    </div>
